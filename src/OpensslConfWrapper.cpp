...
bool OpensslConfWrapper::openConf(const std::string &inputKeyFilename)
{
	int ret = 0;
    long errorline = -1;	
    CONF *tconf = NULL;
    ...
    if(opensslBioWrapper.openBio(inputKeyFilename, 'r', FORMAT_TEXT) == false)
    {
...
bool OpensslConfWrapper::closeConf()
{
	NCONF_free(conf);
	conf = NULL;
